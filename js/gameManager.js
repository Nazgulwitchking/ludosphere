...
const GameManager = {
    ...
    renderLibrary() {
        ...
        // Automatische Übersetzung ausführen, sobald die Karten stehen
    ...
    createGameCard(game) {
        ...
        card.innerHTML = `
            ...
            <button class="favorite-btn" data-game="${game.id}">
                ★
            </button>
        `;
    ...
    toggleFavorite(gameId) {
        const index = this.myGames.indexOf(gameId);
        if(index > -1) {
            this.myGames.splice(index, 1);
        } else {
            this.myGames.push(gameId);
        }
    ...
    saveMyGames() {
        StorageManager.set("myGames", this.myGames);
    },

    loadMyGames() {
        this.myGames = StorageManager.get("myGames", []);
    },
